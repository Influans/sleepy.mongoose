...
class MongoHandler:
    ...
    def _serverStatus(self, args, out, name = None, db = None, collection = None ):
        ...
        if type(args).__name__ == 'dict':
            out('{"ok" : 0, "errmsg" : "_serverStatus must be a POST request"}')
            return
        ...
        #_CMD
        cmd = self._get_son('{"serverStatus": 1 }', out)
        if cmd == None:
            return
